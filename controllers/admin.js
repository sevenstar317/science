 ...
 */
 ...
 var admin = function () {
	...
	/* Discussions */
	var renderDiscussionsPage = function (req, res) {
		async.waterfall([
			function (next) {
				next();
			},
			...
			function (next) {
				SyllabusModel.find({}, {}, {sort: {'order': 1}}, function(err, syllabuses){
					if (err || !syllabuses || syllabuses.length <= 0) {
						...
						next(new Error('Failed to get syllabuses.'));
					} else {
			...
			res.render('admin/discussions', {'syllabuses': syllabuses});
		});
	};

	var getDiscussions = function (req, res) {

		if (!req.param || !req.param('syllabus') ||
        !req.param('grade')) {
        return res.status(500).json(new Error('Some parameters are missing'));
    }
    ...
    if (req.param('chapter') === 'all' || !req.param('chapter')) {
        chapter = 'all';
    } else {
        queryObject.chapter = req.param('chapter');
    }

    async.waterfall([
        // get discussions
        function (cb) {
            DiscussionModel.find(queryObject).lean().exec(function (err, discussions) {
...
var deleteDiscussion = function (req, res) {
	...
	DiscussionModel.find({_id: req.query.id }).remove().exec(function (err) {
		if (err) {
			return res.status(500).json({
        	status_code: 500,
        	message: 'Error'
...
var renderDiscussionEditPage = function (req, res) {
	var id = req.query.id;
  var discus = null;
  async.waterfall([
      // get discussion
...
var editDiscussion = function (req, res) {
	...
	DiscussionModel.findOne({_id: req.body.id}, function (err, discussion) {
		if (err || !discussion) {
			return res.json({
				status_code: 500,
				message: 'Error. Id is missing'
 	...
 	var renderAdminPage = function (req, res) {
		...
		login = function(req, res) {
			...
			async.waterfall([
			function(cb) {
				var config_file = path.join(path.dirname(require.main.filename), '/files/config.json');
		...
		getGradeLink = function(syllabus, notGradeId, gradeNames, retGrades, cb) {
			var query;
		...
		newGrade = function (req, res) {
            ...
            if (err.code === 11000) {
              GradeModel.findOne({grade: schema.grade}, function (err, grade) {
                cb(null, grade._id);
              });
            }
					}
		...
		getChapterLink = function(syllabusId, gradeId, notChapterId, chapterNames, retChapters, cb) {
			var query;
			...
			function(cb) {
				GradeModel.find({}, function(err, grades){
					if (err || !grades || grades.length <= 0) {
						...
						for (var i = 0; i < len; i++) {
							var grade = grades[i];
							gradeNames[grade._id] = grade.grade.toString();
						}
						cb(null);
					}
				});
        ...
        async.each(links, function (linkInfo, next) {
          var exGrades = [];
          var exChapters = [];
          var exConcepts = [];
          async.waterfall([
            ...
            function (cbb) {
              getGradeLink (linkInfo.syllabus.toString(), null, gradeNames, exGrades, function () {
                cbb(null, exGrades);
              });
            },

            function (exGrades, cbb) {
              getChapterLink (linkInfo.syllabus, linkInfo.grade, null, chapterNames, exChapters, function () {
                cbb(null, exGrades, exChapters);
              });
            },

            function (exGrades, exChapters, cbb) {
              getConceptLink (linkInfo.syllabus, linkInfo.grade, linkInfo.chapter, concept_id, conceptNames, exConcepts, function () {
                cbb(null, exGrades, exChapters, exConcepts);
              });
            }

          ], function (err, exGrades, exChapters, exConcepts) {
        ...
        console.log(links);
			...
			function(cb) {
				LinkModel.remove({'kind':'concept', 'concept':concept_id}, function () {
          cb(null);
        });
			},
			function(cb) {
      ...
      var tmp_path = require('path').join(path.dirname(require.main.filename), '/tmp/concept-' + concept_id + ext);
      var image_path = require('path').join(path.dirname(require.main.filename), '/files' + image);
      ...
      var tmp_path = require('path').join(path.dirname(require.main.filename), '/tmp/concept-' + concept_id + '-2nd' + ext);
      var image_path = require('path').join(path.dirname(require.main.filename), '/files' + image);
		...
		getConceptLink = function(syllabusId, gradeId, chapterId, notConceptId, conceptNames, retConcepts, cb) {
			var query;
		...
		newTest = function (req, res) {
			...
			switch(req.param('answer')){
				case 'option1':
				schema.answer = req.param('option1');
				break;
				case 'option2':
				schema.answer = req.param('option2');
				break;
				case 'option3':
				schema.answer = req.param('option3');
				break;
				case 'option4':
				schema.answer = req.param('option4');
				break;
			}
        ...
        function(cb) {
          ChapterModel.find({}, {'title': 1}, {sort: {'order': 1}}, function(err, chapters) {
            if (err || !chapters || chapters.length <= 0)
              return res.render('admin/search-contents', param);
            else
		...
		searchContents = function(req, res) {
          ...
          //params.syllabus = {$in: query_syllabuses};
					params.syllabus = req.param('syllabuses');
				//}
			}
			var query_grades = [];
			if (req.param('grades')) {
          ...
          console.log(params.syllabus);
				...
				function(cb) {
					ConceptModel.find({
						...
						if (err || !concepts || concepts.length <= 0) {
							console.log('Failed to get concept for search contents.1');
							return res.send({
								'status_code': 400,
								'message': 'Not found contents'
								});
            ...
            async.each(links, function (item, next) {
              var concept = conceptCash[item.concept];

              if (tmp_conceptId === concept._id) {
                return next()
              }

              getLinkByTypeId2('concept', concept, function(resConcept, resLinks){
            ...
            async.each(links, function (item, next1) {
              var concept = conceptCash[item.concept];
              ...
              function(err, videos){
                if (err || !videos || videos.length <= 0) {
                  ...
                  async.each(videos, function (video, next2) {
                    getLinkByTypeId2('concept', concept, function(resConcept, resLinks){
                      if (resLinks && resLinks.length > 0) {
                        var linkInfo = [];
            ...
            async.each(links, function (item, next1) {
              ...
              function(err, references){
                if (err || !references || references.length <= 0) {
                  ...
                  async.each(references, function (reference, next2) {
		...
		getLinkByTypeId2 = function(kind, item, callback) {
		...
		getGradeListAllUnmapped2 = function(isAll, retGrades, callback) {
			syllabusNames = [];
			...
			function(grades, cb) {
				...
				for (var i = 0; i < len; i++) {
					...
					getLinkByTypeId2('grade', grade, function(resGrade, links){
		...
		getChapterListAllUnmapped2 = function(isAll, retChapters, callback) {
			syllabusNames = [];
			gradeNames = [];
			...
			function(chapters, cb) {
				...
				for (var i = 0; i < len; i++) {
					...
					getLinkByTypeId2('chapter', chapter, function(resChapter, links){
		...
		getChapters = function(req, res) {
			...
			function(cb) {
				if (option === 'all') {
					...
					getChapterListAllUnmapped2 ('false', chapters, function(){
						cb(null,chapters);
					});
				}
			},
		...
		getConceptListAllUnmapped2 = function(isAll, retConcepts, callback) {
			syllabusNames = [];
			gradeNames = [];
			chapterNames = [];
			...
			function(concepts, cb) {
				...
				for (var i = 0; i < len; i++) {
					...
					getLinkByTypeId2('concept', concept, function(resConcept, links){
		...
		getConcepts = function(req, res) {
			...
			function(cb) {
				if (option === 'all') {
					...
					getConceptListAllUnmapped2 ('false', concepts, function(){
						cb(null,concepts);
					});
				}
			},
	...
	return {
		...
		// discussions
		renderDiscussionsPage: renderDiscussionsPage,
		getDiscussions: getDiscussions,
		deleteDiscussion: deleteDiscussion,
		renderDiscussionEditPage: renderDiscussionEditPage,
