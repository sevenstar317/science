...
var home = function () {
    ...
    var UserModel = require('../models/users'),
		...
		oauth2Client = new OAuth2('45714664599-s92itaj8vrsnqq88uj4v6eagu3e93rsh.apps.googleusercontent.com', 'k6ZSQMf_zbHbHhy0j8DiapqK', 'http://scienceisfun.mobiloitte.org:3000/auth/google/callback');
		...
		renderLastLessonPage = function (req, res) {
			console.log('called Last Page');
			res.redirect('/lessons');
			//res.redirect('/afterLogin');
		},
		renderLessonsPage = function (req, res) {
			console.log('Hit on Lesson');
					...
					TestScoremodel.find({'user':userId,'test_title':test_title,'chapter':chapterId},function(err,testscore){
						if(err||!testscore){
							...
							testsInfo = {'status_code':200, 'test_count':testIds.length, 'testIds':testIds, "tests":tests, "usertest_data":""}
								...
								testsInfo = {'status_code':200, 'test_count':testIds.length, 'testIds':testIds, "tests":tests, "usertest_data":testscore}
								}
								res.send(testsInfo);
						});
				}
					...
					if (req.user.provider == "facebook") {
						...
						facebook.get(req.user.accessToken, '/me/friends', function(data) {
							...
							for (i = 0; i < len; i++ ) {
								...
								resSocialfriends.push({
									name: users_info.name,
									photo :'https://fbcdn-profile-a.akamaihd.net/hprofile-ak-xpf1/v/t1.0-1/c15.0.50.50/p50x50/10354686_10150004552801856_220367501106153455_n.jpg?oh=82c2ce067f9c7f2c0a66d1efb16730e7&oe=5670F72F&__gda__=1450287355_17925d7381c8da5663c2349483f4b032',
									type:"facebook"
									});
								t++;
							}
						...
						plus.people.list({ userId: 'me','collection': 'visible', auth: oauth2Client }, function(err, response) {
							...
							for(i=0; i < numItems; i++) {
								resSocialfriends.push({'name':response.items[i].displayName,'photo':response.items[i].image.url});
							}
							cb(null,resSocialfriends);	
						});
					}
		...
		getPercentage = function(req,res){
			...
			function(cb){
				LinkModel.find({'kind':'test','grade':grade,'syllabus':syllabus},function (err, tests){
					...
					totaltest = Math.floor(len/6);
					cb(null,totaltest);
				});
			},
			function(totaltest,cb){
				TestScoremodel.find({'user':userid},function (err, testscore){
					if(err||!testscore)
					console.log('Did not get score');
					else
					testlen = testscore.length;
					cb(null,totaltest,testlen);
				});
			},
			function(totaltest,testlen,cb){
				percentage = (testlen/totaltest)*100;
				percentageInfo.push({'userid':userid,'percentage':percentage});
		...
		getPercentile = function(req,res){
			...
			var counter2 =0;
			var counter3 =0;
			var calutatepercentile;
			var percentileInfo = [];
			
			async.waterfall([
				function(cb){
					TestScoremodel.aggregate([{ $match : {'user': userid } },
											  { $group: { _id: userid, count: { $sum: '$score' } } }],
					function (err, testscore){
						if(err||!testscore)
							{}
						else
						testscore.forEach(function(k){
								myscore = k.count;
							});
								cb(null,myscore);
						});
					...
					TestScoremodel.aggregate(
					   [
						...
						if(err||!testuser)
							...
							var len = testuser.length;
							for(var i=0;i<len;i++){
								testusers[i] = testuser[i]._id;
								}
								cb(null,myscore,testusers);
							});
				},
				function(myscore,testusers,cb){
					var tlen = testusers.length; 
					for(var i= 0;i<tlen;i++){
						 var jj = testusers[i];
						 var user = jj.toString();
						UserModel.find({'_id': user,'school_name':userschool,'syllabus':syllabus,'grade':grade},
							function(err, users){
								if(err||!users)
								{}
								else
								users.forEach(function(k){
									...
									TestScoremodel.aggregate([
									{ $match: { 'user': user } },
									{ $group: { sum: { $sum: '$score' }, _id: null } }],
									function(err, scoreuser){
											if(err||!scoreuser)
											{}
										else
										scoreuser.forEach(function(m){
											counter2+=1;
											if(m.sum<=myscore){
												counter+=1;
												...
												cb(null,counter,counter2,counter3);
					...
					function(counter,counter2,counter3,cb){
						
						calutatepercentile = (counter/counter2)*100;
						var percentile = calutatepercentile.toFixed(0);
