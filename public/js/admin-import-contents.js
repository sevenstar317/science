...
$(document).ready(function(){
	...
	$('form').submit(function(event) {
		...
		if (content_type === 'syllabus')
			reqUrl = '/admin/contents/import/mapping';
		else
			reqUrl = '/admin/contents/import/concept';

		$('#processing').text('0%');
		$('#wait').removeClass('hidden');
		$.ajax({
			//url: '/admin/contents/import',
			url: reqUrl,
			type: 'POST',
	...
	socket.on('resImporting', function(data) {
		var percent = 0;
	...
	socket.on('resImportCompleted', function(data) {
		$('#wait').addClass('hidden');
